...
Cond.Class.D<-function(df,conf){
  ...
  cond.crit=data.frame(Class=c("Insuficient data",
                       ...
                       criteria=c(
                         ...
                         'all.na>0',
                         #Critical
                         'Coral.cover=="No" & Recovery.performance=="No" & Processes=="TwoOrThree.No"',
                         
                         #Warning II
                         'Coral.cover=="No" & Recovery.performance=="Yes" & Processes=="TwoOrThree.No"',
                         'Coral.cover=="Yes" & Recovery.performance=="No" & Processes=="TwoOrThree.No"',
                         'Coral.cover=="No" & Recovery.performance=="No" & Processes=="One.No"',

                         
                         #Warning I
                         'Coral.cover=="No" & Recovery.performance=="Yes" & Processes=="One.No"',
                         'Coral.cover=="Yes" & Recovery.performance=="No" & Processes=="One.No"',
                         'Coral.cover=="Yes" & Recovery.performance=="Yes" & Processes=="TwoOrThree.No"',
                         'Coral.cover=="No" & Recovery.performance=="No" & Processes=="All.Yes"',
                         
                         #Watch
                         'Coral.cover=="No" & Recovery.performance=="Yes" & Processes=="All.Yes"',
                         'Coral.cover=="Yes" & Recovery.performance=="Yes" & Processes=="One.No"',
                         'Coral.cover=="Yes" & Recovery.performance=="No" & Processes=="All.Yes"',
  ...
  if (dim(df)[1]==0){
    ...
    c.df<-df%>%
      ...
      crit=case_when(
        ...
        #((Ind.g %in% c("Processes")  &  crit.no==3)) ~ "All.No",
        ((Ind.g %in% c("Processes")  &  crit.no==0)) ~ "All.Yes",
        ((Ind.g %in% c("Processes")  & crit.no ==1)) ~ "One.No",
        ((Ind.g %in% c("Processes")  & crit.no %in% (c(2,3)))) ~ "TwoOrThree.No",
        .default = NA)
      )%>%
      select(-crit.no)%>%
      spread(key=Ind.g, val=crit)%>%
